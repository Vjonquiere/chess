...
public class ZobristHashing {
   ...
   *
   * @param simplified if true : the hash computer later will only contain the pieces positions.
   *     Else, it will be composed of the pieces, the en passant files, the castling rights and
   *     current player.
   */
  public ZobristHashing(boolean simplified) {
    initializeHash();
   ...
   * rights are encoded as a 4-bit integer: - Bit 0 (1) - White can castle kingside - Bit 1 (2) -
   * White can castle queenside - Bit 2 (4) - Black can castle kingside - Bit 3 (8) - Black can
   * castle queenside
   *
  ...
  /**
   * Generate the hash corresponding to the current board state. If simplified : it will only
   * contain the pieces.
   *
   ...
   * of the piece that moved, the piece captures and if it is not simplified the updated castling
   * rights, en passant files and player.
   *
