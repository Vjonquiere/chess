...
# --- Constants ---
POP_SIZE = 60
GENS = 200  # Run for multiple generations
MUTATION_RATE = 0.4
CROSSOVER_RATE = 0.7
...
CSV_FILEPATH = "./results_new.csv"
csv_lock = threading.Lock()

logging.basicConfig(
    filename='error_new.log',
    level=logging.ERROR,
...
def save_checkpoint(population, current_gen, hof):
    ...
    with open("checkpoint_new.pkl", "wb") as f:
        pickle.dump(checkpoint_data, f)
    print(f"Checkpoint saved at generation {current_gen}")

def load_checkpoint():
    try:
        with open("checkpoint_new.pkl", "rb") as f:
...
def run_single_game(weights, first_white):
    ...
    if engine2_is_white and game_result == "1-0" or not engine2_is_white and game_result == "0-1":
        bonus_moves_win = (1.0 / moves if moves > 0 else 0.0)
        bonus_captures_win = max(0, (captures_white-captures_black if engine2_is_white else captures_black-captures_white))/39

        bonus_win = bonus_moves_win*MOVES_WEIGHT + bonus_captures_win*CAPTURES_WEIGHT
        score += bonus_win

    elif engine2_is_white and game_result == "0-1" or not engine2_is_white and game_result == "1-0":
        bonus_moves_lose = (1.0 * moves/200 if moves < 200 else 1.0)
        bonus_captures_win = max(0, (captures_white-captures_black if engine2_is_white else captures_black-captures_white))/39

        bonus_lose = bonus_moves_lose*MOVES_WEIGHT + bonus_captures_win*CAPTURES_WEIGHT
        score += bonus_lose
...
def adaptive_tournament(individuals, k):
    tournsize = 2 + int(CURRENT_GEN / 20)
    return tools.selTournament(individuals, k, tournsize=min(5, tournsize))

def mutate_with_catastrophic(individual, mu, indpb):
    individual, = mutGaussian(individual, mu, indpb)
    
    # 5% chance
    if random.random() < 0.05:
        idx = random.randrange(len(individual))
        individual[idx] = random.uniform(*WEIGHT_RANGE)
...
toolbox.register("mutate", mutate_with_catastrophic, mu=0, indpb=MUTATION_RATE)
toolbox.register("select", adaptive_tournament)

def calculate_similarity(ind1, ind2):
    return sum((a-b)**2 for a, b in zip(ind1, ind2))
    ...
    for i, ind1 in enumerate(population):
        ...
        for j, ind2 in enumerate(population):
            if i != j:
                ...
                if distance < 20: 
                    sharing_sum += 1 - (distance/20)
        if sharing_sum > 0:
            ind1.fitness.values = (ind1.fitness.values[0] / (1 + sharing_sum),)
    return population
...
def main():
    ...
    for gen in range(CURRENT_GEN, GENS):
        ...
        if stagnation_counter >= 15:
            print("Stagnation detected. Replacing the worst 80% of the population with new individuals...")
            num_to_replace = int(0.8 * len(pop))
            worst_indices = sorted(range(len(pop)), key=lambda i: pop[i].fitness.values[0])[:num_to_replace]
            random_individuals = toolbox.population(n=num_to_replace)
            
            for idx, new_ind in zip(worst_indices, random_individuals):
                pop[idx] = new_ind
            stagnation_counter = 0

            new_individuals = [pop[idx] for idx in worst_indices]
            parallel_eval(new_individuals)
