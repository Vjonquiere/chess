...
Nous générons donc au départ des nombres aléatoires pour chaque combinaison possible de case et de pièce, que nous mettons dans le tableau `PIECES`.
...
à partir du moment où on l'a calculée une fois. Le gain de temps est considérable car les états de jeu peuvent se répéter un grand nombre de fois.
...
Nous avons effectué plusieurs refactor afin d'arriver à une architecture plus claire plusieurs fois au corus du projet. Dans les dernières semaines de développement, 
lorsqu'il y avait moins de nouvelles features ajoutées, nous nous sommes penchés sur les outils de qualité de code tels que `Checkstyle` ou `PMD`.
Les règles utilisées dans PMD sont disponibles dans le fichier `pmd-rules.xml`.
